...
public class TPCABench extends Thread {
  ...
  Driver m_Driver;
  int m_nNumRuns;
  AtomicInteger m_nTrans = new AtomicInteger(0);
  ...
  AtomicLong m_nTimeSum = new AtomicLong(0);
  long m_time;
  ...
  protected void setBarValue(int newValue)
  {
      SwingUtilities.invokeLater(
              new Runnable() {
                  public void run() {
                      bar.setValue(m_nTrans.get());
                      double tpS = m_nTrans.doubleValue();
                      double timeSum = Double.longBitsToDouble(m_nTimeSum.get());
                      if (timeSum > 0)
                          tpS = tpS / timeSum;
                      else
                          tpS = 0;
                      tpsLabel.setText(txnForm.format(m_nTrans.get()) + " txn (" + tpsForm.format(tpS) + " tps)");
                      if (m_time > 0 && pane != null && pane.m_tpcaBench != null) // if the bench runs for ... minutes
                      { // do rescaling of progress bars based on an estimate numer of transactions for the specified time
                          ...
                          {
                              nCurTrans = ((TPCABench)el.nextElement()).m_nTrans.get();
  ...
  public double getTotalTime() {
    return Double.longBitsToDouble(m_nTimeSum.get());
  }
      ...
      try {
          ...
          for (int nRun = 0; (m_time>0)?true:nRun < m_nNumRuns; nRun++) {
              ...
              if (m_time>0) {
                  ...
                  if ((current - m_time)>(m_nNumRuns* 60000L)) {
                      break;
                  } else {
              ...
              try {
                  ...
                  AtomicDoubleAdd(m_nTimeSum, (diff/1000.0));
          ...
          for (int nRun = 0; ((m_time>0) ? true : nRun < m_nNumRuns); nRun++) {
              ...
              if (m_time>0) {
                  ...
                  if ((current - m_time) > (m_nNumRuns * 60000L)) {
                      ...
                      if (nRun % prop_update_freq == 0)
                          setBarValue((int)((current - m_time)/1000.0));
                  }
              } else if (nRun % prop_update_freq == 0)
                  setBarValue(nRun);
              ...
              try {
                  ...
                  AtomicDoubleAdd(m_nTimeSum, (double)(diff / 1000));
      ...
      try {
          ...
          for (int nRun = 0; ((m_time>0) ? true : nRun < m_nNumRuns); nRun++) {
              ...
              try {
                  ...
                  long endTime = System.currentTimeMillis();
                  m_nTrans.addAndGet(m_BatchSize);
                  long diff = endTime - startTime;
  ...
  void runPrepareTest_Batch() throws SQLException
  {
      ...
      try {
          ...
          for (int nRun = 0; ((m_time>0) ? true : nRun < m_nNumRuns); nRun++) {
              ...
              try {
                  ...
                  AtomicDoubleAdd(m_nTimeSum, (diff/1000));
    ...
    double AtomicDoubleAdd(AtomicLong atom, double v)
    {
        while (true) {
            long current = atom.get();
            double currentVal = Double.longBitsToDouble(current);
            double nextVal = currentVal + v;
